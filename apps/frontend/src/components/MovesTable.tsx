      ...
      <div className="text-sm h-[45vh] max-h-[45vh] overflow-y-auto" ref={movesTableRef}>
        {movesArray.map((movePairs, index) => {
          return (
            ...
            >
              <div className="grid grid-cols-6 gap-16 w-4/5 py-2 bg-[#171717] rounded-lg">
                ...
                {movePairs.map((move, movePairIndex) => {
                  ...
                  return (
                    <div
                      key={movePairIndex}
                      className={`col-span-2 cursor-pointer flex items-center w-full pl-1 ${isHighlighted ? 'bg-[#484644] rounded border-b-[#5A5858] border-b-[3px]' : ' rounded border-b-[#5A5858] border-b-[3px]'}`}
                      onClick={() => {
