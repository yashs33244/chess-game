...
export class GameManager {
  ...
  private addHandler(user: User) {
    user.socket.on('message', async (data) => {
      ...
      if(message.type === CHAT) {
        this.handleChatMessage(user, message.payload);
      }
  ...
  private async handleChatMessage(user: User, payload: ChatPayload) {
    const roomId = socketManager.getUserRoom(user.userId);
    if (!roomId) {
      console.error(`User ${user.userId} not in any room.`);
      return;
    }
  
    if (!payload.message || typeof payload.message !== 'string') {
      console.error(`Invalid payload from user ${user.userId}:`, payload);
      return;
    }
    ...
    try {
      socketManager.broadcast(roomId, JSON.stringify(chatMessage));
    } catch (error) {
      console.error(`Failed to broadcast message from user ${user.userId}:`, error);
    }
  }
